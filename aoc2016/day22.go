...
func main() {
	nodes := parse(loadData("day22.txt"))
	fmt.Println(viablePairs(nodes))
}
...
func parse(lines []string) []node {
	var nodes []node
	re := regexp.MustCompile("/dev/grid/node-x([0-9]+)-y([0-9]+)")
	for _, line := range lines {
		var n node
		words := strings.Fields(line)
		pos := re.FindStringSubmatch(words[0])
		...
		n.pos.x = x
		n.pos.y = y
		size, err := strconv.Atoi(strings.TrimSuffix(words[1], "T"))
		if err != nil {
			log.Fatal(err)
		}
		n.Size = size
		used, err := strconv.Atoi(strings.TrimSuffix(words[2], "T"))
		if err != nil {
			log.Fatal(err)
		}
		n.Used = used
		avail, err := strconv.Atoi(strings.TrimSuffix(words[3], "T"))
		if err != nil {
			log.Fatal(err)
		}
		n.Avail = avail
